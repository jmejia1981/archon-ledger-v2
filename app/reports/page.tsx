...
export default function ReportsPage() {
    ...
    const loadReportsData = async () => {
      try {
        ...
        invoices.forEach((inv: any) => {
          ...
          if (!monthlyObj[monthKey]) {
            monthlyObj[monthKey] = { month: monthKey, revenue: 0, expenses: 0 }
          }
        ...
        const sortedMonthlyData = Object.values(monthlyObj).sort((a, b) =>
        ...
        const projDist = projects.map((p: any) => ({
          name: p.project_name,
          value: invoices
            .filter((inv: any) => inv.project_id === p.id)
            .reduce((sum, inv) => sum + (inv.invoice_amount || inv.amount || 0), 0),
        }))
        ...
        laborEntries.forEach((entry: any) => {
          ...
          if (!deptLabor[dept]) {
            deptLabor[dept] = { name: dept, value: 0 }
          }
          deptLabor[dept].value += cost
        })

        const laborData = Object.values(deptLabor)

        setMonthlyData(sortedMonthlyData)
        setProjectDistribution(projDist)
        setLaborByDept(laborData)
        setMetrics({
          totalRevenue,
          totalExpenses,
          netProfit,
  ...
  if (loading) {
    return <div className="animate-pulse space-y-6"><div className="h-10 w-64 bg-gray-200 rounded"></div></div>
  }
    ...
    <div className="space-y-6" style={{ backgroundColor: '#F5F5F5', padding: '32px' }}>
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        >
          ...
          <span>Jan 01, 2024 - May 09, 2026</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          ...
          >
            ...
            <p className="text-2xl font-bold" style={{ color: '#1A3A6B' }}>
              {kpi.isPercent ? `${kpi.value.toFixed(1)}%` : formatCurrency(kpi.value)}
            </p>
            <p className="text-xs" style={{ color: '#10B981', marginTop: '8px' }}>
      ...
      <div className="flex gap-6 mb-6 border-b" style={{ borderColor: '#E0E0E0' }}>
        {['Overview', 'Financial', 'Operations', 'Custom Reports'].map((tab) => (
          <button
            ...
            style={{
              color: activeTab === tab.toLowerCase() ? '#1A3A6B' : '#999',
              borderBottom: activeTab === tab.toLowerCase() ? '2px solid #1A3A6B' : 'none',
            }}
          >
            {tab}
          </button>
      ...
      <div
        ...
        style={{ backgroundColor: 'white', border: '1px solid #E0E0E0' }}
      >
        <h3 className="text-base font-semibold mb-4" style={{ color: '#1A3A6B' }}>
          Revenue vs Expenses
        </h3>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div
          ...
          style={{ backgroundColor: 'white', border: '1px solid #E0E0E0' }}
        >
          <h3 className="text-base font-semibold mb-2" style={{ color: '#1A3A6B' }}>
            Project Distribution
          </h3>
          ...
          {projectDistribution.length > 0 ? (
            ...
            <div className="h-[220px] flex items-center justify-center bg-gray-100">
              <p className="text-gray-400">No data available</p>
            </div>
          )}
